...
class SleeperNetsBackdoorCallback(BaseCallback):
    """
    Poison a fraction of episodes by turning on spoofing during a window.
    The environment implements the actual spoofing/backdoor behavior.
    """
    ...
    def _on_rollout_start(self) -> None:
        ...
        if self.base_env and getattr(self.base_env, 'backdoor_training', False):
            if random.random() < self.config.poisoning_rate:
...
class ReLaraTrainingCallback(BaseCallback):
    """
    Periodically trains the ReLaRa potential network during SAC rollouts.
    """
    ...
    def _on_step(self) -> bool:
        if not self.relara_agent or self.relara_agent.beta == 0:
            return True

        self.steps_since_train += 1
        if self.steps_since_train >= self.train_freq:
            self.relara_agent.train_potential_function(batch_size=64)
            self.steps_since_train = 0
        return True
